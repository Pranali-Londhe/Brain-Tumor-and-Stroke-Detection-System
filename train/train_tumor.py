...
Usage:
    python train_tumor.py --data_dir /path/to/tumor_data --arch transfer --epochs 25

--arch custom   -> from-scratch CNN (models/tumor_cnn.py: build_custom_cnn)
--arch transfer -> EfficientNetB0 transfer learning (recommended)
"""
...
from models.tumor_cnn import build_custom_cnn, build_transfer_model, compile_model, IMG_SIZE
from train.dataset_utils import load_datasets, build_augmentation


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--data_dir", required=True, help="Path to tumor MRI dataset")
    parser.add_argument("--arch", choices=["custom", "transfer"], default="transfer")
    parser.add_argument("--epochs", type=int, default=25)
    parser.add_argument("--batch_size", type=int, default=32)
    parser.add_argument("--lr", type=float, default=1e-4)
    parser.add_argument("--out", default="../saved_models/tumor_model.keras")
    ...
    with open(os.path.join(os.path.dirname(args.out), "tumor_class_names.txt"), "w") as f:
        f.write("\n".join(class_names))
